...
UPSTREAMS = {
    ...
    # 如有新增上游，可在此添加，但仅影响 README 中的描述，不影响规则生成
}
...
def generate(entries: list) -> str:
    ...
    for name, url in UPSTREAMS.items():
        upstream_lines.append(f'- [{name}]({url})')
    upstream_text = '\n'.join(upstream_lines)
...
{{
  "type": "remote",
  "tag": "规则名称",
  "url": "{CDN_BASE}/singbox/规则名称/规则名称.srs",
  "format": "binary"
}}
```

> 若 CDN 访问不畅，可替换为 raw 地址：  
> `{RAW_BASE}/singbox/规则名称/规则名称.srs`
...
rule-providers:
  规则名称_domain:
    ...
    behavior: domain
    format: mrs
    url: "{CDN_BASE}/mihomo/规则名称/规则名称_domain.mrs"
    interval: 86400
  规则名称_ipcidr:
    type: http
    behavior: ipcidr
    format: mrs
    url: "{CDN_BASE}/mihomo/规则名称/规则名称_ipcidr.mrs"
    interval: 86400
```

> 如需使用 raw 链接，请将 `{CDN_BASE}` 替换为 `{RAW_BASE}`。
