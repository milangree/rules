...
for dir in "$mihomo_base"/*/; do
    ...
    c_yaml="$dir/${name}_classical.yaml"
    if [[ -f "$c_yaml" ]]; then
        c_mrs="$dir/${name}_classical.mrs"
        if mihomo convert-ruleset classical yaml "$c_yaml" "$c_mrs" 2>/dev/null; then
            echo "  ✓  [mihomo/classical]  $name"
            ((mh_ok++)) || true
        else
            echo "  ✗  [mihomo/classical]  $name（编译失败）"
            ((mh_fail++)) || true
        fi
    fi
