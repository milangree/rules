...
# 全局变量，稍后在 download_ljrgov_repo 中动态赋值
ljrgov_rules_path = None
...
def download_ljrgov_repo():
    ...
    # 动态检测解压后的顶层目录名
    extracted_items = os.listdir(source_folder)
    top_dirs = [d for d in extracted_items if os.path.isdir(os.path.join(source_folder, d))]
    if not top_dirs:
        logging.critical('ljrgov/conf 解压后未找到任何目录')
        exit(1)
    top_dir = top_dirs[0]   # 例如 'conf-main'
    # 修正：仓库根目录下直接有 sing-box 文件夹，没有额外的 conf 层
    ljrgov_rules_path = os.path.join(top_dir, 'sing-box', 'rules')
    logging.info(f'动态检测 ljrgov 规则路径：{ljrgov_rules_path}')
...
def _parse_singbox_json(path: str) -> tuple:
    ...
    try:
        with open(path, 'r', encoding='utf-8-sig') as f:   # 处理 UTF-8 BOM
            data = json.load(f)
...
def translate_ljrgov():
    ...
    """
    if ljrgov_rules_path is None:
        logging.error('ljrgov_rules_path 未初始化，请确认 download_ljrgov_repo 已执行')
        return
    ...
    if not os.path.isdir(rules_dir):
        ...
        # 打印实际目录结构便于调试
        base = os.path.join(current_dir, 'ljrgov_conf')
        if os.path.exists(base):
            logging.info(f'实际解压内容：{os.listdir(base)}')
            for root, dirs, files in os.walk(base):
                logging.debug(f'  {root} : {dirs} {files[:5] if files else []}...')
        return
    ...
    for fname in sorted(os.listdir(rules_dir)):
        ...
        # 可选：跳过 Ads_fix（如果不需要）
        # if name == 'Ads_fix,AI-fix,Direct-fix,ads-add,ads_fix':
        #     logging.info(f'[ljrgov] 跳过 {name}（已配置忽略）')
        #     continue
        ...
        if not any([domain, domain_keyword, domain_suffix, ip_cidr, process_name]):
            ...
            try:
                ...
                with open(path, 'r', encoding='utf-8-sig') as f:
                    preview = f.read(200)
                logging.debug(f'{name} 文件大小 {size} 字节，预览: {preview[:100]}...')
            except Exception as e:
                logging.debug(f'无法读取 {name}: {e}')
            continue
...
extra_surge_conf = {}   # 可在此添加额外规则，格式：{'规则名': 'https://...'}
